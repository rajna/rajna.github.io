...
/***/ function(module, exports, __webpack_require__) {
	...
	$(function () {
		...
		$('.searchSelect').change(function (event) {
			var v = $.trim($(this).children('option:selected').text());
			$(this).parents('.search-item').find('.searchVal').html(v);
		});
		...
		$(window).scroll(function (e) {
			...
			var h2 = $('.partbanner').height();
			if ($(window).scrollTop() > h1 + h2) {
				$('.nav-content').css({ 'position': 'fixed', 'top': '0px', 'left': '0px', 'right': '0px', 'z-index': '99999' });
			} else {
				$('.nav-content').css({ 'position': 'relative' });
			}
